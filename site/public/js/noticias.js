...
for (var i = 0; i < listaNoticias.length; i++) {
  addNoticias(
    listaNoticias[i].idNoticia,
    listaNoticias[i].imagem,
    listaNoticias[i].assunto,
    listaNoticias[i].manchete,
    listaNoticias[i].descricao
  );
}

var paginaAtual = 0;
var qtdNoticiasAdicionadas = 0;
function verMaisNoticias(tamanhoPagina) {
  for (var i = 0; i < tamanhoPagina; i++) {
    if (qtdNoticiasAdicionadas == listaNoticias.length) {
      document.getElementById(
        "botaoVerMais"
      ).innerHTML = `<a href="#">Voltar ao topo</a>`;
      break;
    }
    var indice = paginaAtual * tamanhoPagina + i;
    var noticia = `<div class="noticia" id="${listaNoticias[indice].idNoticia}">`;
    noticia += ` <div class="imagem">
              <img src="${listaNoticias[indice].imagem}" alt="">
            </div>
            <div class="conteudo">
              <h4>${listaNoticias[indice].assunto}</h4>
              <span class="manchete">${listaNoticias[indice].manchete}</span>
              <span class="descricao">${listaNoticias[indice].descricao}</span>
            </div>
          </div>
          `;
